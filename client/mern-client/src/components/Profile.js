...
function Profile(props) {
    ...
    const roll = sessionStorage.getItem("curr_roll_number");

    // If roll is null it means the session variable is not set and hence the user 
    ...
    // control comes here if roll is not null.
    return (<div>
        <button className='btn btn-primary m-4' onClick={async (e) =>  {
                ...
                if(res.status == 200) {
                    sessionStorage.removeItem("curr_roll_number");
                    navigateToLogin();
                }
            }}>Logout</button>
